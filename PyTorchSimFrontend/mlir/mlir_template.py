...
class MLIRTemplateKernel(MLIRKernel, BaseMLIRHardwareInfo):
    ...
    def prepare_input(self, indent_size: int = 0):
        def hook():
            ...
            code = self.prologue_buffer_group.body.getvalue()
            # FIXME: This cannot distinguish between input and weight buffers
            if 0 not in self.prologue_info['apply_buf']:
                code += f"memref.dma_start %X[%index0], %X_buffer[%c0, %c0], %c_mvin, %tag1[%c0], %axis, %vstride: memref<{self.prologue_info['input_size'][0] * self.prologue_info['input_size'][1]}xf32>, memref<{self.prologue_info['input_tile_size'][0]}x{self.prologue_info['input_tile_size'][1]}xf32, 1>, memref<1xi32> {{ subtile_size=[{self.prologue_info['input_subtile_size'][0]}, {self.prologue_info['input_subtile_size'][1]}], async=1, sram_stride=[1, {self.prologue_info['input_tile_size'][0]}] }}\n"
            if 1 not in self.prologue_info['apply_buf']:
                code += f"memref.dma_start %W[%index1], %W_buffer[%c0, %c0], %c_mvin2, %tag2[%c0], %axis, %vstride: memref<{ self.prologue_info['weight_size'][0]*self.prologue_info['weight_size'][1] }xf32>, memref<{self.prologue_info['weight_tile_size'][0]}x{self.prologue_info['weight_tile_size'][1]}xf32, 1>, memref<1xi32> {{ subtile_size=[{self.prologue_info['weight_subtile_size'][0]}, {self.prologue_info['weight_subtile_size'][1]}], async=1, sram_stride=[1, {self.prologue_info['weight_tile_size'][0]}] }}\n"
            code = textwrap.indent(code, " "*indent_size).strip()
            return code
    ...
    def load_prologue(self, name: str, index: sympy.Expr):
        ...
        tile_shape = self.kernel_group.tile_desc.get_mlir_shape(mlir_dtype)
        tile_stride = self.prologue_info['input_tile_stride']
    ...
    def store_prologue(self, name: str, index: sympy.Expr, value, *args, **kwargs):
        ...
        sram_var = self.prologue_info['arg_mapping'][name]
        zero_var = self.get_const_cse(0)
