...
class my_opt_decoder(nn.Module):
    def __init__(self, config: LLM_Config, current_seq_len):
        ...
        self.register_buffer(
            "past_k",
            torch.randn(bsz, num_heads // self.config.tp, current_seq_len, self.head_dim)
        )
        self.register_buffer(
            "past_v",
            torch.randn(bsz, num_heads // self.config.tp, current_seq_len, self.head_dim)
        ...
        # QKV layers
        self.k_proj = nn.Linear(self.config.embed_dim, self.config.embed_dim // self.config.tp, bias=self.config.enable_bias)
        self.v_proj = nn.Linear(self.config.embed_dim, self.config.embed_dim // self.config.tp, bias=self.config.enable_bias)
        self.q_proj = nn.Linear(self.config.embed_dim, self.config.embed_dim // self.config.tp, bias=self.config.enable_bias)
        self.o_proj = nn.Linear(self.config.embed_dim // self.config.tp, self.config.embed_dim, bias=self.config.enable_bias)
        ...
        self.fc1 = nn.Linear(self.config.embed_dim, config.ffn_dim // self.config.tp, bias=config.enable_bias)
        self.fc2 = nn.Linear(self.config.ffn_dim // self.config.tp, self.config.embed_dim, bias=config.enable_bias)
    ...
    def qkv(self, hidden_states):
        ...
        query_states = query_states.view(self.bsz, -1, self.config.num_heads // self.config.tp, self.head_dim).transpose(1, 2)

        key_states = self.k_proj(hidden_states)
        value_states = self.v_proj(hidden_states)
        key_states = key_states.view(self.bsz, -1, self.config.num_heads // self.config.tp, self.head_dim).transpose(1, 2)
        value_states = value_states.view(self.bsz, -1, self.config.num_heads // self.config.tp, self.head_dim).transpose(1, 2)
    ...
    # out-proj + rms
    def out_proj(self, attn_output, residual):
        self.bsz = attn_output.size(0)
        self.tgt_len = attn_output.size(1)
...
if __name__ == "__main__":
    ...
    embed_dim = 2048 // args.tp
    hidden_size = 2048 // args.tp
    num_heads = 32 // args.tp
    ffn_dim = 8192
    vocab_size = 50272
    ...
    hidden = torch.randn(
        bsz, 1, config.num_heads // config.tp, config.hidden_size // config.num_heads,
        dtype=torch.float32
    )
